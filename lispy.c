...
#define LASSERT(cond, x, y, msg) if (!(cond)) { lval_del(x); lval_del(y); x = lval_err(msg); break; }
...
enum { LVAL_ERR, LVAL_DUB, LVAL_NUM, LVAL_SYM, LVAL_SEXPR };
...
lval* builtin_op(lval* a, char* op) {
  ...
  while (a->count > 0) {
    ...
    if (x->type == LVAL_NUM) {
      ...
      if (strcmp(op, "/") == 0) {
        LASSERT((y->num != 0), x, y, "Division By Zero!");
        x->num /= y->num;
      }
      if (strcmp(op, "%") == 0) {
        LASSERT((y->num != 0), x, y, "Division By Zero!");
      ...
      if (strcmp(op, "/") == 0) {
        LASSERT((y->dub != 0.0), x, y, "Division By Zero!");
        x->dub /= y->dub;
      }
      if (strcmp(op, "%") == 0) {
        LASSERT((y->dub != 0.0), x, y, "Division By Zero!");
